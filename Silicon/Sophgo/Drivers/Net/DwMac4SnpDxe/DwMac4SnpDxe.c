  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "EFI_PAGES_TO_SIZE (sizeof (DMA_DESCRIPTOR))=0x%lx\n", EFI_PAGES_TO_SIZE (sizeof (DMA_DESCRIPTOR))));
#if 0 
    //
    // Flush Rx Descriptor
    //
  ...
  )
{
  ...
  if (EFI_ERROR (Status)) {
    DEBUG ((
      DEBUG_ERROR,
      "%a(): gCpu->SetMemoryAttributes Status = %r !\n",
      __func__,
      Status
      ));
  }
  ...
  )
{
  ...
  for (Index = 0; Index < TX_DESC_NUM; Index++) {
    ...
    Status = DmaAllocateBuffer (EfiBootServicesData,
	                        ...
	                        (VOID *)&DwMac4Driver->MacDriver.TxDescRing[Index]
				);
    if (EFI_ERROR (Status)) {
      DEBUG ((
	DEBUG_ERROR,
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((
        DEBUG_ERROR,
        "%a(): Tx gCpu->SetMemoryAttributes Status = %r !\n",
        __func__,
        Status
        ));
    }
    ...
    Status = DmaAllocateBuffer (EfiBootServicesData,
                                EFI_SIZE_TO_PAGES (DescriptorSize),
				(VOID *)&DwMac4Driver->MacDriver.RxDescRing[Index]
				);
    if (EFI_ERROR (Status)) {
      DEBUG ((
        DEBUG_ERROR,
    ...
    if (EFI_ERROR (Status)) {
      DEBUG ((
        DEBUG_ERROR,
        "%a(): Rx gCpu->SetMemoryAttributes Status = %r !\n",
        __func__,
        Status
        ));
    }
  ...
  //
  DwMac4Driver->MacBase = 0x7030006000;
 ...
 // }
  ...
  if (EFI_ERROR(Status)) {
    ...
    DEBUG ((
      DEBUG_ERROR,
      "%a(): Install SIMPLE_NETWORK_PROTOCOL failed!\n",
      __func__
      ));
  }
